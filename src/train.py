...
from src.datasets.ucla import make_ucla
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, (udata, masks_enc, masks_pred) in enumerate(unsupervised_loader):
            ...
            def train_step():
                ...
                return (float(loss), float(pred_loss), float(reg_loss), _new_lr, _new_wd, grad_stats)
            (loss, pred_loss, reg_loss, _new_lr, _new_wd, grad_stats), etime = gpu_timer(train_step)
            ...
            def log_stats():
                ...
                if (itr % log_freq == 0) or np.isnan(loss) or np.isinf(loss):
                    logger.info('[%d, %5d] loss: %.3f pred: %.3f reg: %.3f '
                                    'masks: %.1f %.1f '
                                    '[wd: %.2e] [lr: %.2e] '
                                    '[mem: %.2e] '
                                    '(%.1f ms)'
